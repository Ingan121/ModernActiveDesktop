	...
	function $Component(title, className, orientation, $el) {
		...
		const render_ghost = (e) => {
			...
			$ghost.css({
				...
				width: w - inset * 2,
				height: h - inset * 2,
				left: e.clientX / madScaleFactor + ($dock_to ? ox : ox2) + inset,
				top: e.clientY / madScaleFactor + ($dock_to ? oy : oy2) + inset,
			});
		...
		$c.add($w.$titlebar).on("pointerdown", e => {
			...
			ox = rect.left - e.clientX / madScaleFactor;
			oy = rect.top - e.clientY / madScaleFactor;
			...
			ox2 = rect.left - offsetLeft - e.clientX / madScaleFactor;
			oy2 = rect.top - offsetTop - e.clientY / madScaleFactor;
		...
		const drag_update_position = e => {
			...
			$ghost.css({
				left: e.clientX / madScaleFactor + ox,
				top: e.clientY / madScaleFactor + oy,
			});
		...
		const drag_onpointerup = e => {
			...
			if ($dock_to) {
				...
				undock_to(e.clientX / madScaleFactor + ox2, e.clientY / madScaleFactor + oy2);
			}
