...
function to_canvas_coords({ clientX, clientY }) {
	...
	return {
		x: ~~((clientX / madScaleFactor - rect.left) / rect.width * main_canvas.width),
		y: ~~((clientY / madScaleFactor - rect.top) / rect.height * main_canvas.height),
	};
}
...
async function init_eye_gaze_mode() {
	...
	const on_pointer_move = (e) => {
		recent_points.push({ x: e.clientX / madScaleFactor, y: e.clientY / madScaleFactor, time: Date.now() });
	};
	...
	const get_event_options = ({ x, y }) => {
		return {
			...
			clientX: x / madScaleFactor,
			clientY: y / madScaleFactor,
...
$canvas_area.on("pointerdown", (event) => {
	...
	if (pointers.every((pointer) =>
		...
		pointers.push({
			...
			x: event.clientX / madScaleFactor,
			y: event.clientY / madScaleFactor,
		});
	}
...
$G.on("pointermove", (event) => {
	for (const pointer of pointers) {
		if (pointer.pointerId === event.pointerId) {
			pointer.x = event.clientX / madScaleFactor;
			pointer.y = event.clientY / madScaleFactor;
		}
	}
	if (pointers.length >= 2) {
