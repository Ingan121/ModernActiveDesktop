...
You can also disable this warning by passing {iframes: {ignoreCrossOrigin: true}} to $Window.`));
	...
	var update_drag = (e) => {
		...
		if (
			...
			drag_pointer_x = e.clientX / madScaleFactor ?? drag_pointer_x;
			drag_pointer_y = e.clientY / madScaleFactor ?? drag_pointer_y;
		}
		$w.css({
	...
	$w.$titlebar.on("pointerdown", (e) => {
		...
		drag_offset_x = e.clientX / madScaleFactor + scrollX - $w.position().left;
		drag_offset_y = e.clientY / madScaleFactor + scrollY - $w.position().top;
		drag_pointer_x = e.clientX / madScaleFactor;
		drag_pointer_y = e.clientY / madScaleFactor;
	...
	if (options.resizable) {
		...
		[
			...
			$handle.on("pointerdown", (e) => {
				...
				resize_offset_x = e.clientX / madScaleFactor + scrollX - rect.x - (x_axis === HANDLE_RIGHT ? rect.width : 0);
				resize_offset_y = e.clientY / madScaleFactor + scrollY - rect.y - (y_axis === HANDLE_BOTTOM ? rect.height : 0);
				resize_pointer_x = e.clientX / madScaleFactor;
				resize_pointer_y = e.clientY / madScaleFactor;
